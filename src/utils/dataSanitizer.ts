  ...
  try {
    ...
    for (const d of invSnap.docs) {
      ...
      if (data.status ==="completed" && data.payoutDetails) {
        if (
          typeof data.payoutDetails.totalCredited !=="number" ||
          isNaN(data.payoutDetails.totalCredited) ||
          data.payoutDetails.totalCredited > 100000000
        ) {
          data.payoutDetails.totalCredited = data.amount + data.amount * 0.05;
          updated = true;
        }
        if (
          typeof data.payoutDetails.rmasCommission !=="number" ||
          isNaN(data.payoutDetails.rmasCommission) ||
          data.payoutDetails.rmasCommission > 10000000
        ) {
    ...
    for (const inv of validInvestments) {
      if (inv.status ==="active") {
        totalInvestedByInvestor.set(
          inv.investorId,
          (totalInvestedByInvestor.get(inv.investorId) || 0) + inv.amount,
        );
      }
    }
    ...
    for (const d of bizSnap.docs) {
      ...
      if (
        typeof data.fundingRequired !=="number" ||
        isNaN(data.fundingRequired) ||
        data.fundingRequired > 1000000000 ||
        data.fundingRequired < 0
      ) {
    ...
    for (const d of invsSnap.docs) {
      ...
      if (
        ...
        typeof data.totalInvested !=="number" ||
        isNaN(data.totalInvested)
      ) {
        data.totalInvested = calculatedTotal;
        updated = true;
      }
    ...
    return {
      success: true,
      message: `Sanitized and recalculated ${sanitizedCount} records in the database successfully.`,
    };
  } catch (error) {
