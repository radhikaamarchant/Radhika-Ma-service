...
export const getVerificationStats = (
...
): Map<string, VerificationStats> => {
  const statsMap = new Map<string, VerificationStats>();

  // Group investments by business and by owner
  const businessStats = new Map<
    string,
    { totalInvested: number; totalProfit: number; uniqueInvestors: Set<string> }
  >();
  const ownerStats = new Map<
    string,
    { totalInvested: number; totalProfit: number }
  >();

  // Initialize maps
  businesses.forEach((b) => {
    businessStats.set(b.id, {
      totalInvested: 0,
      totalProfit: 0,
  ...
  investments.forEach((inv) => {
    if (inv.status ==="completed" && inv.payoutDetails) {
      ...
      if (b) {
        ...
        const totalReturn =
          p.totalCredited + p.rmasCommission + p.happyIncomeTax;
  ...
  businesses.forEach((b) => {
    ...
    const businessProfitPct =
      bStat.totalInvested > 0 ? bStat.totalProfit / bStat.totalInvested : 0;
    // Condition 2: Total number of unique investors >= 20
    const businessQualifies =
      businessProfitPct >= 0.6 && bStat.uniqueInvestors.size >= 20;

    const isBlueTick = businessQualifies || b.id ==="admin_business";
    const isPreVerified =
      !isBlueTick && businessProfitPct >= 0.3 && businessProfitPct < 0.6;

    // Progress calculation towards the 60% mark
    // If profit is 30%, progress is 30/60 = 0.5
    const progressToBlueTick = Math.min(
      Math.max(businessProfitPct / 0.6, 0),
      1,
    );
...
export const getBlueTickBusinessIds = (
  businesses: Business[],
  investments: Investment[],
): Set<string> => {
