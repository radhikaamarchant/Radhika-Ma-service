...
type Action =
  ...
  | {
      ...
      payload: { id: string; status: Business["status"] };
    }
  ...
  const dispatch = async (action: Action) => {
    try {
      ...
      switch (action.type) {
        case"ADD_BUSINESS":
          await setDoc(
            doc(db,"businesses", action.payload.id),
        ...
        case"ADD_INVESTOR":
          await setDoc(
            doc(db,"investors", action.payload.id),
            payloadWithTimestamp,
          );
          break;
        ...
        case"ADD_INVESTMENT":
          await setDoc(
            doc(db,"investments", action.payload.id),
            payloadWithTimestamp,
          );
          break;
    ...
    <AppContext.Provider value={{ state, dispatch }}>
      {state.loading ? (
        <div className="flex h-screen w-full items-center justify-center">
          <p className="text-kite-text-light font-medium tracking-wider uppercase text-sm">
            Loading System...
          </p>
        </div>
      ) : (
