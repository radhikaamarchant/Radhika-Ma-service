...
const oldHeaderRegex = /<div className="px-0 flex flex-col md:flex-row md:justify-between md:items-end relative mb-3 md:mb-0">[\s\S]*?<\/div>\n\s*<\/div>/;

const newHeader = `<div className="px-3 md:px-0 flex flex-col md:flex-row md:justify-between md:items-end relative mb-3 md:mb-0">
  <div className={\`flex flex-col md:flex-row w-full items-start md:items-center justify-between transition-all duration-300 gap-3 md:gap-0 \${isSearchExpanded ? 'opacity-0 pointer-events-none absolute' : 'opacity-100'}\`}>
    ...
    <div className="flex flex-col md:flex-row items-start md:items-center w-full md:w-auto md:justify-end gap-2 md:gap-4">
      ...
      <div className="w-full md:w-auto flex justify-start md:justify-end md:order-2 border-b border-kite-border/50 md:border-none pb-2 md:pb-0">
      ...
      <div className="w-full md:w-auto md:order-1 pt-1 md:pt-0">
  ...
  <div className={\`flex items-center w-full transition-all duration-300 bg-white md:bg-transparent rounded-sm \${isSearchExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 pointer-events-none translate-x-4 absolute'}\`}>
    <button onClick={() => { setIsSearchExpanded(false); setSearchTerm(''); }} className="p-2 -ml-2 hover:bg-gray-100 rounded-full mr-1 transition-colors flex-shrink-0">
